class PurchaseFileParser
  ...
  def create_purchases(path_to_purchase_file, purchase_upload)
    ...
    File.readlines(path_to_purchase_file).each_with_index do |line, index|
      ...
      Purchase.create(purchase_upload: purchase_upload, customer: customer, merchant: merchant, item_description: item_description, item_price: item_price, purchase_count: purchase_count)
    end
